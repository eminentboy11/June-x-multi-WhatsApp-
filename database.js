...
function createBotDatabase(options = {}) {
  const botId = String(options.botId || 'june-x-main');
  const DB_FILE = options.dbFile
    ? path.resolve(options.dbFile)
    : path.resolve(process.env.JUNE_DB_FILE || path.join(DB_DIR, 'june-ultra.db'));
  const DB_BACKUP_FILE = options.backupFile
    ? path.resolve(options.backupFile)
    : `${DB_FILE}.backup`;
  const pg = options.pg || pgAdapter;
  const mongo = options.mongo || mongoAdapter;
  ...
  function getRemoteAdapter(name) {
    return name === 'postgres' ? pg : name === 'mongo' ? mongo : null;
  }
  ...
  function hasConfiguredRemoteAuthMirror() {
    return [pg, mongo].some((adapter) => Boolean(adapter?.getStatus?.().configured));
  }
  ...
  async function restoreRemoteAuthState() {
    ...
    try {
      ...
      const candidates = (await Promise.all([
        typeof pg.fetchAuthState === 'function' ? pg.fetchAuthState() : null,
        typeof mongo.fetchAuthState === 'function' ? mongo.fetchAuthState() : null,
  ...
  async function restoreFromPostgres() {
    const result = await pg.restoreIntoSQLite(db);
    if (result?.restored > 0) requestBackup('postgres-restore');
    return result;
  }

  async function restoreFromMongo() {
    const result = await mongo.restoreIntoSQLite(db);
...
const api = {
  ...
  resetDatabase,
  getPostgresStatus: pg.getStatus, getMongoStatus: mongo.getStatus,
  getBotId: pg.getBotId,
};

Object.defineProperty(api, '_db', {
  enumerable: true,
  get() {
    return db;
  }
...
// ─── Multi-session database facade ───────────────────────────────────────────
// The single-session API is preserved: `require('./database')` behaves exactly
// like the original module for the default bot. Each additional bot session
// gets its own SQLite file (june-<botId>.db) and its own instance API, and
// every call made inside a bot context (utils/botContext AsyncLocalStorage)
// is routed to that bot's database automatically.

const botContext = require('./utils/botContext');
const CONTEXT_DEFAULT_BOT_ID = botContext.DEFAULT_BOT_ID;

const registry = new Map();

function defaultDbFileFor(botId) {
  if (botId === CONTEXT_DEFAULT_BOT_ID) return undefined; // legacy path/file
  return path.join(DB_DIR, `june-${String(botId).replace(/[^a-zA-Z0-9_.-]/g, '-')}.db`);
}

function registerBotDatabase(botId, options = {}) {
  const id = String(botId || CONTEXT_DEFAULT_BOT_ID);
  if (registry.has(id)) return registry.get(id);
  const resolved = {
    botId: id,
    dbFile: options.dbFile || defaultDbFileFor(id),
    backupFile: options.backupFile,
    pg: options.pg || pgAdapter.forBot(id),
    mongo: options.mongo || mongoAdapter.forBot(id),
  };
  const instance = createBotDatabase(resolved);
...
async function shutdownAllDatabases() {
  const results = await Promise.allSettled(
    [...registry.values()].map((instance) => instance.shutdownDatabase())
  );
  return results;
}
...
function closeAllConnections() {
  for (const instance of registry.values()) {
    try { instance._db?.close?.(); } catch (_) {}
  }
}

// Legacy default instance — created at require time exactly like the original
// single-session database, using the legacy database file path.
const defaultBotDatabase = registerBotDatabase(CONTEXT_DEFAULT_BOT_ID);
...
const facade = new Proxy({
  ...
  get(target, prop) {
    ...
    const db = target.getBotDatabase(botContext.getCurrentBotId()) || defaultBotDatabase;
    return db[prop];
  },
});
